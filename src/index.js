...
const Index = memo(props => {
  ...
  useEffect(() => {
    ...
    myGolabelChange = props.attributes
    ...
    myGolabelPlaceholder = props.attributes ? (props.attributes.placeholder ? props.attributes.placeholder : 'select...') : 'select...';
  ...
  useEffect(() => {
    if (!stateShowOptions[0]) {
      ...
      stateOptionsHash[1]([...getOptionsHash(myGolabelOptionsInit, myGolabelSelectedName)]);
      stateInputValue[1]('');
    }
    // 监听原生scroll
    if (myGolabelInit) {
      ...
      // 监听点击scroll
      scrollBarWrapperRef.current.addEventListener('mousedown', handleClickWrapper);
    ...
    if (optionsRef.current.clientHeight !== optionsRef.current.scrollHeight) {
      ...
      scrollBarRef.current.style.height = `${(optionsRef.current.clientHeight / optionsRef.current.scrollHeight) * 100}%`;
    }
    myGolabelInit = false;
  }, [stateShowOptions[0]]);
  ...
  const handleClickWrapper = e => {
    if (e.target === scrollBarWrapperRef.current) {
      ...
      const scrollbarClickPosition = ((offset - scrollbarHalf) * 100) / e.target.offsetHeight;
      optionsRef.current.scrollTop = (scrollbarClickPosition * optionsRef.current.scrollHeight) / 100;
    }
  };
  ...
  const handleScroll = () => {
    const moveY = (optionsRef.current.scrollTop / optionsRef.current.clientHeight) * 100;
    scrollBarRef.current.style.transform = `translateY(${moveY}%)`;
  };

  const handleClickBar = e => {
    startDrag(e);
    myGolabelMouseY = e.currentTarget.offsetHeight - (e.clientY - e.currentTarget.getBoundingClientRect().top); // 获取鼠标点击位置
  };
  ...
  const handleMouseMove = e => {
    ...
    const offset = (scrollBarWrapperRef.current.getBoundingClientRect().top - e.clientY) * -1; // 获取鼠标相对容器偏移量
    const scrollbarClickPosition = scrollBarRef.current.offsetHeight - myGolabelMouseY;
    const scrollbarPositionPercentage = ((offset - scrollbarClickPosition) * 100) / scrollBarWrapperRef.current.offsetHeight; // 获取鼠标相对点击位置偏移量（offset - scrollbarClickPosition）
    optionsRef.current.scrollTop = (scrollbarPositionPercentage * optionsRef.current.scrollHeight) / 100;
  };
  ...
  const handleSelect = obj => {
    if (!obj.disabled) {
      ...
      myGolabelIconClass = obj.icon ? obj.icon : '';
      stateIconClass[1](myGolabelIconClass);
      stateInputPlaceholder[1](myGolabelSelectedName !== '' ? myGolabelSelectedName : myGolabelPlaceholder);
      if (obj.name === '') {
        stateInputValue[1]('');
      }
  ...
  const handleChangeInputValue = val => {
    ...
    stateIconClass[1](val === '' ? myGolabelIconClass : '');
      ...
      case 13: {
        if (myGolabelEnterStatus === true) {
          ...
          if (myGolabelHoverIndex !== -1 && myGolabelOptionsHash.length !== 0) {
            ...
            const tempHash = getOptionsHash(myGolabelOptionsInit, myGolabelSelectedName);
            myGolabelOptionsHash = tempHash;
            stateOptionsHash[1](tempHash);
            stateShowOptions[1](false);
          }
  ...
  const handleHover = (e, type) => {
    ...
    // 计算当前houver
    if (type === 'prev') {
      myGolabelHoverIndex = myGolabelHoverIndex < 1 ? myGolabelOptionsHash.length - 1 : myGolabelHoverIndex - 1;
    } else {
      myGolabelHoverIndex = myGolabelHoverIndex > myGolabelOptionsHash.length - 2 ? 0 : myGolabelHoverIndex + 1;
    }
    stateHoverIndex[1](myGolabelHoverIndex);
    // 计算滚动条位置
    if (optionRef.current !== null && optionsRef.current.clientHeight !== optionsRef.current.scrollHeight) {
      if (type === 'prev') {
        const offset = optionRef.current.getBoundingClientRect().top - optionsRef.current.getBoundingClientRect().top;
        ...
        const offset = optionRef.current.getBoundingClientRect().bottom - optionsRef.current.getBoundingClientRect().bottom;
